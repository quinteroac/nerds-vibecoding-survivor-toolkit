...
- Commands output instructions as prompts to be executed by modern AI environments (Cursor, Antigravity, Claude Code Web) instead of implicitly calling agents internally. Prompts and skills are loaded from `.agents/skills/<name>/SKILL.md`.
...
- **Ideation Phase** (optional preliminary phase)
- **Requirement Phase**: `nvst define requirement` / `refine requirement` / `approve requirement` (interactive clarification and challenge mode)
- **Prototype Phase**: `nvst create prototype` (outputs prompts for implementation, creates branches `feature/it_XXXXXX-[Req-Description]`)
- **Audit Phase**: `nvst audit prototype`
- **Refactor Phase**: `nvst refactor prototype`
- **Approval Phase**: `nvst approve prototype` (automates context/roadmap updates, git pushes, and PR creation)
- **Utilities**: `nvst write-json`, project-context related flows
- Execution mode: Prompt-based output built for modern agentic IDEs, seamlessly supporting team environments.
