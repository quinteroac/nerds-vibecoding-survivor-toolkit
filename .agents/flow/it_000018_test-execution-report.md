...
| TC-005 | When stdin is not a TTY (or confirmFn simulates non-TTY), confirmFn returns false immediately without waiting for input. | failed | US-001, FR-6 | `.agents/flow/it_000018_test-execution-artifacts/TC-005_attempt_001.json` |
...
| TC-007 | Typecheck passes after implementation (`bun tsc --noEmit`). | skipped | US-001, FR-1, FR-2 | `.agents/flow/it_000018_test-execution-artifacts/TC-007_attempt_001.json` |
...
| TC-011 | Typecheck passes. | skipped | US-002 | `.agents/flow/it_000018_test-execution-artifacts/TC-011_attempt_001.json` |
...
| TC-015 | Typecheck passes. | skipped | US-003 | `.agents/flow/it_000018_test-execution-artifacts/TC-015_attempt_001.json` |
...
| TC-019 | All tests in the project pass (`bun test`); existing create-prototype tests that hit the dirty-tree path use an injected confirmFn (e.g. returning false) so they still pass. | skipped | US-004, FR-7 | `.agents/flow/it_000018_test-execution-artifacts/TC-019_attempt_001.json` |
| TC-020 | Typecheck passes. | skipped | US-004 | `.agents/flow/it_000018_test-execution-artifacts/TC-020_attempt_001.json` |
