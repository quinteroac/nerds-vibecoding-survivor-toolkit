...
- Generate requirement document (it_000001_product-requirement-document.md)
  ...
  - Command **`bun nvst refine requirement`**: Validates: `requirement_definition.status` is "in_progress" and the file in `requirement_definition.file` exists. Runs: invokes the **refine-pr-document** skill; the agent updates that .md. Does not change state. Can be run multiple times.
